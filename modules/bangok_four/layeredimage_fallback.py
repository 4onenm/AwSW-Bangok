...
# Normally this value would come from the Ren'Py `config.py`,
#  but it doesn't exist yet in 6.99.
config_layeredimage_offer_screen = True
...
class LayeredImage(object):
    ...
    def __init__(self, attributes, at=[], name=None, image_format=None, format_function=None, attribute_function=None, offer_screen=None, **kwargs):
        ...
        self.transform_args = {k : kwargs.pop(k) for k, v in list(kwargs.items()) if k not in (renpy.sl2.slproperties.position_property_names + [style.name for style in renpy.sl2.slproperties.box_properties])}
    ...
    def _duplicate(self, args):
        ...
        if offer_screen is None:
            offer_screen = config_layeredimage_offer_screen
        if offer_screen:
        ...
        # Version 6.99 of Ren'Py does not support
        #  linting flags in DisplayableArguments
